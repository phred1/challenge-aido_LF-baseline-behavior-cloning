...
class LogViewer:
    ...
    def update_image(self):
        ...
        self.frame_index += 1
        sample = cv2.cvtColor(sample, cv2.COLOR_YUV2RGB)
        img_array = Image.fromarray(sample)
        img_array.thumbnail(self.size, Image.ANTIALIAS)
        img_array.save(f"./dataset/{ENV}/{ENV}_{self.counter}.jpeg")

        if self.counter == 38751:
            sys.exit(0)
